...
select sysT.name, type = case is_instead_of_trigger
							when 0 then 'AFTER'
							when 1 then 'INSTEAD_OF'
						 end
  from sys.triggers sysT, sys.objects sysO
 where sysT.object_id = sysO.object_id and
       sysT.parent_id = (select object_id 
					       from sys.objects 
					      where name = 'Parts');
go
