...
begin
    ...
    loop
        ...
        case 
            when not HasUpper and instr('ABCDEFGHIJKLMNOPQRSTUVWXYZ', l_Char) != 0
                then HasUpper := true;
            when not HasLower and instr('abcdefghijklmnopqrstuvwxyz', l_Char) != 0
                then HasLower := true;
             when not HasDigit and instr('0123456789', l_Char) != 0
                then HasDigit := true;
        end case;
